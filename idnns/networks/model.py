...
class Model:
        ...
        def __init__(self, input_size, layerSize, num_of_classes, learning_rate_local=0.001, save_file='',
                     activation_function=0, conv_net=False):
                self.convnet = conv_net
                self.input_size = input_size
                self.layerSize = layerSize
                self.all_layer_sizes = np.copy(layerSize)
                self.all_layer_sizes = np.insert(self.all_layer_sizes, 0, input_size[0])
        ...
        def hidden_layers(self):
                ...
                if self.hidden is None:
                        ...
                        last_hidden = self.x
                        if self.convnet:
                                raise Exception('they made it impossible to get here')
                                ...
                                last_hidden = self.initialize_layer(name_scope, row_size, col_size, activation_function, last_hidden)
                return self.hidden
