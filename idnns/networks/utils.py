...
def load_data(name, random_labels=False):
        ...
        if name.split('/')[-1] == 'MNIST':
                print(f'the name should be {os.path.dirname(sys.argv[0])}')
                print(f'the name should be {sys.argv}')
                data_sets_temp = input_data.read_data_sets(os.path.dirname(sys.argv[0]) + "data/MNIST/", one_hot=True)
                ...
                print(data_sets.labels[:20,:])
        elif name.split('/')[-1] == 'cifar10':
                data_sets.data, data_sets.labels = get_cifar_data()
        else:
...
def get_cifar_data():
        #TODO if this gets shared...
        baseDirectory ='/home/david/projects/IDNNs/data/cifar10/cifar-10-batches-py/'
        filename_list = [baseDirectory+'data_batch_1', baseDirectory+'data_batch_2', baseDirectory+'data_batch_3', baseDirectory+'data_batch_4', baseDirectory+'data_batch_5', baseDirectory+'test_batch']
        dicts = []
        for name in filename_list:
                with open(name,'rb') as fo:
                        dicts.append(pickle.load(fo,encoding='bytes'))
        data = np.concatenate([x[b'data'] for x in dicts],axis=0)
        labelTmp = np.concatenate([x[b'labels'] for x in dicts],axis=0)
        labels = np.zeros((labelTmp.shape[0],10))
        for i in range(labelTmp.shape[0]):
                labels[i,labelTmp[i]] = 1.0
        return (data, labels)
