...
from idnns.information.mutual_info_estimation import calc_variational_information
...
import sys
sys.path.append("/home/david/projects/NPEET/")
...
def calc_information_for_layer_with_other(data, bins, unique_inverse_x, unique_inverse_y, label,
        ...
        """
           calculate the information between X and y
        """
...
def calc_information_this_epoch(iter_index, interval_information_display, ws_iter_index, bins, unique_inverse_x,
                               unique_inverse_y, label, b, b1,
                               len_unique_a, pys, pxs, py_x, pys1, model_path, input_size, layerSize, calcMethod, k,
                               num_of_samples=100, sigma=0.5, ss=[], ks=[]):
        """Calculate the information for all the layers for specific epoch"""
        np.random.seed(None)
        if calcMethod =='calc_full_and_vartional':
                # Vartional information
                params_vartional = [
                        calc_variational_information(ws_iter_index[i], label, model_path, i, len(ws_iter_index) - 1, iter_index,
        ...
        elif calcMethod=='calc_variational_information':
                # use the variational method of estimating the mutual information
                params = [
                        calc_variational_information(ws_iter_index[i], label, model_path, i, len(ws_iter_index) - 1, iter_index,
                                                    input_size, layerSize, ss[i], pys, ks[i], search_sigma=True) for i in
                        range(len(ws_iter_index))]
        # Calc information of only subset of the neurons
        elif calcMethod == 'calc_information_by_sampling':
        ...
        elif calcMethod == 'calc_regular_information':
                params = np.array(
                        [calc_information_for_layer_with_other(data=ws_iter_index[i], bins=bins, unique_inverse_x=unique_inverse_x,
                                                               unique_inverse_y=unique_inverse_y, label=label,
                                                               b=b, b1=b1, len_unique_a=len_unique_a, pxs=pxs,
...
# y = np.random.binomial(2,.5,100)
# y_sep = np.zeros(y.shape + (3,))
# for i,val in enumerate(y):
#         y_sep[i,val] = 1
# pys, pys1, p_y_given_x, b1, b, unique_a, unique_inverse_x, unique_inverse_y, pxs = extract_probs(y_sep,x)
# unique_inverse_x
# x.dtype.itemsize
...
def extract_probs(label, x):
        """
           ...
           returns:
             pys : the probability of a given class; assumes each column refers to a datapoint
             pys1 :
             p_y_given_x :
             b1 :
             b : a view of each x vector as a void type with as many bytes as the equivalent # of entries would have
             unique_a :
             unique_inverse_x : a list of indices such that b = unique_array[uniuqe_inverse_x]
        """
...
def calc_information_from_knn(w, x, label, iter_index, interval_information_display, ws_iter_index, bins, unique_inverse_x,
                              unique_inverse_y, b, b1,
                              len_unique_a, pys, pxs, py_x, pys1, model_path, input_size, layerSize, calcMethod, k,
                              num_of_samples=100, sigma=0.5, ss=[], ks=[]):
        """Calculate the information for all the layers for specific epoch"""
        np.random.seed(None)
...
def get_information(ws, x, label, num_of_bins, interval_information_display, model, layerSize, calcMethod,
                    calc_parallel=True, py_hats=0,k=3):
        ...
        if calcMethod=='knn':
                print('not ready yet')
                #params = np.array([calc_knn_information() for i in range(len(ws))])
        if calc_parallel:
                params = np.array(Parallel(n_jobs=NUM_CORES
                                             ...
                                             (i, interval_information_display, ws[i], bins, unique_inverse_x, unique_inverse_y,
                                              ...
                                              pxs, p_y_given_x, pys1, model.save_file, x.shape[1], layerSize, calcMethod, k)
...
def minimal_get_information(transformed, label, save_file,layerSize, calcMethod, num_of_bins=30, interval_information_display=30, calc_parallel=True, py_hats=0,k=3):
        bins = np.linspace(-1, 1, num_of_bins)
        label = np.array(label).astype(np.float)
        pys, pys1, p_y_given_x, b1, b, unique_a, unique_inverse_x, unique_inverse_y, pxs = extract_probs(label, transformed[0])
        params = calc_information_this_epoch(i, interval_information_display, transformed, bins, unique_inverse_x, unique_inverse_y, label, b, b1, len(unique_a), pys, pxs, p_y_given_x, pys1, model.save_file, transformed[0].shape[1], layerSize, calcMethod, k)
        return params
